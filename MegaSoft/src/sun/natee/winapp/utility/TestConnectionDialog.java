...
//        test.mapToClassBean("product");
    ...
    private void loadTable() {
        ...
        try {
            //Load driver mysql
            Class.forName("com.mysql.jdbc.Driver");
            try (Statement stmt = conn.connectOpen(txtURL.getText()).createStatement()) {
    ...
    private void loadColumn() {
        ...
        try {
            ...
            try (Statement stmt = conn.connectOpen(txtURL.getText()).createStatement()) {
                stmt.execute("use " + cbDB.getSelectedItem());
                try (ResultSet rs = stmt.executeQuery("describe " + cbTB.getSelectedItem())) {
                    DefaultListModel mmodel = new DefaultListModel();
                    listColumn.setModel(mmodel);            
                    for(int i=0;i<mmodel.getSize();i++){
                        mmodel.remove(0);
                    }
                    while (rs.next()) {
                        mmodel.addElement(rs.getString(1));
                    }
                }
            }
            conn.close();
    ...
    private String[] getColumns() {
        ...
        try {
            //Load driver mysql
            Class.forName("com.mysql.jdbc.Driver");
            try (Statement stmt = conn.connectOpen(txtURL.getText()).createStatement(); ResultSet rs = stmt.executeQuery(txtSQL.getText())) {
    ...
    private String[] mapToClassBean(String table) {
        ...
        try {
            //Load driver mysql
            Class.forName("com.mysql.jdbc.Driver");
            try (Statement stmt = conn.connectOpen(txtURL.getText()).createStatement(); ResultSet rs = stmt.executeQuery("describe "+table)) {
                String columnName;
                String columnType;
                ...
                buff.append("public class ").append(table).append("Bean{\n");      
                while(rs.next()){
                    ...
                    buff.append("     private ").append(columnType).append(" ").append(columnName).append(other).append(";\n");
                }
